...
_OPTIONAL_NUMERIC_QUERY_FIELDS = {"max_competitors", "delayFrom", "delayTo", "senderId", "company", "category_id", "region_id"}
...
async def strip_empty_optional_numeric_filters(request, call_next):
    ...
    if raw_query:
        ...
        cleaned = [(key, value) for key, value in pairs if not (key in _OPTIONAL_NUMERIC_QUERY_FIELDS and value.strip() == "")]
        if cleaned != pairs:
            request.scope["query_string"] = urlencode(cleaned, doseq=True).encode("utf-8")
    return await call_next(request)

install_ai_panel(app, fetch_all_orders=fetch_all_orders, compact_order=compact_order, filter_orders=filter_orders, api_filter_dict=api_filter_dict, has_my_offer=has_my_offer, max_competitors=max_competitors, esc=esc)
